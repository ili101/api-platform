...
final class SymfonyScaffold
{
    ...
    public const SYMFONY_DOCKER_REF = 'main';
    ...
    public function run(string $projectDir, string $projectName, ScaffoldOptions $opts): int
    {
        ...
        if ($opts->withDocker) {
            $this->setupDocker($apiDir, $opts->symfonyDockerRef);
        }
    ...
    private function setupDocker(string $apiDir, string $ref): void
    {
        ...
        try {
            ...
            $this->runner->run(['git', '-C', $tmpDir, 'fetch', '--depth=1', '--quiet', 'origin', $ref]);
